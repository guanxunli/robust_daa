...
library(parallel)
## load parameters
para0 <- readRDS("loglinear_data/outlier4/datasets/log.normal.para.rds")
beta0 <- para0$beta0
sigma2 <- para0$sigma2
# parameter use
...
for (iter_para in seq_len(nrow(setting))) {
  ...
  dta_list <- mclapply(seq_len(n_sim), function(iter_simu) {
    ...
    ## generate X
    tmp <- beta %*% t(Z) + beta0
    error_mat <- matrix(rgamma(m * n, shape = 2, rate = 1.25), nrow = m)
  ...
  saveRDS(dta_list, paste0(
    "loglinear_data/outlier4/datasets/nocon_n", n,
...
for (iter_para in seq_len(nrow(setting))) {
  ...
  # save datasets
  saveRDS(dta_list, paste0(
    "loglinear_data/outlier4/datasets/con_n", n,
    "gamma", gamma, "mu", mu_use, ".rds"
  ))
}

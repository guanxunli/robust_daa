...
library(parallel)
## load parameters
para0 <- readRDS("mixeffect_data/outlier2/datasets/log.normal.para.rds")
beta0 <- para0$beta0
sigma2 <- para0$sigma2
# parameter use
...
for (iter_para in seq_len(n_setting)) {
  ...
  dta_list <- mclapply(seq_len(n_sim), function(iter_simu) {
    ...
    r <- matrix(rlnorm(m * n.id, meanlog = log(1), sdlog = log(1.5)), nrow = m)[, id]
    r <- r - mean(r)
    Z <- cbind(u, id)
    beta <- alpha
  ...
  saveRDS(dta_list, paste0(
    "mixeffect_data/outlier2/datasets/n", n,
